"""
Test the strength and validity of our generated instruments. 
"""
...
for county in merged_data.county.unique():
    ...
    # z = np.polyfit(merged_data[merged_data.county == county]["charging_density_L2_other_counties"], merged_data[merged_data.county == county]["charging_density_L2"], 1)
    # p = np.poly1d(z)
    # ax.plot(merged_data[merged_data.county == county]["charging_density_L2_other_counties"], p(merged_data[merged_data.county == county]["charging_density_L2_other_counties"]), "r--", color = 'lightgrey')
ax.legend()
...
Y = merged_data["charging_density_L2"]
X = merged_data["bartik_instrument_L2"]
X = sm.add_constant(X)
...
cols_fe = ["bartik_instrument_L2", "t"] + [x for x in merged_data.county.unique().tolist()[1:]]
